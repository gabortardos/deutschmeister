...
## Status: M0 ✅ · M0.1 ✅ · M1 ✅ · M1.1 ✅ · M1.2 ✅ · M2 ✅ · M2.1 ✅ · M2.2 ✅ · M2.3 ✅ · M3 ✅ · M4.1 ✅ · M4 ✅ — **v1.0.0 complete** 🎉 · **Phase 2: M4.2 ✅ (v1.0.1) · M5.1 ✅ (v1.1.0) · M5.2 ✅ (v1.1.1) · M5.3 ✅ (v1.1.2) · M6.1 ✅ (v1.2.0) · M6.2 ✅ (v1.2.1) · M6.3 ✅ (v1.2.2) · M7.1 ✅ (v1.3.0) · M7.2 ✅ (v2.0.0) · M8 ✅ (v2.1.0) · M8.1 ✅ (v2.1.1) · M8.2 ✅ (v2.2.0) · M8.3 ✅ (v2.3.0) · M9 code ✅ (v2.4.0/v2.4.1 — owner sandbox deployment in progress) · M9.7 ✅ (v2.4.6) · M9.5 ✅ (v2.5.0) · M9.6 ✅ (v2.6.0) · M10–M11 ⬜** — full plan: `docs/PHASE2_PLAN.md`
...
| M8.2 z.ai relay + optional platform swap | ✅ done | (this commit) | BYO relay in `ai-proxy` (POST `/byo/zai-coding` or `/byo/zai-api` `/chat/completions` → api.z.ai server-side; caller's key in `x-dm-byo-key`, forwarded once, never stored; host-locked routes, POST-only, 64 KB cap, 30 req/min/IP, 60 s timeout; anon JWT in Authorization satisfies platform verify) so z.ai / GLM Coding Plan keys work again — no Chinese account needed; new default provider `glm-zai` ("Zhipu GLM (z.ai / Coding Plan)", glm-4.6, `relay:` sentinel baseUrl + zai-api alt probe) + self-heal migrates pre-M2.2 api.z.ai rows onto it; adapter relay plumbing (`resolveRelayBaseUrl` sentinel→functions URL, anon-JWT + x-dm-byo-key headers); optional platform swap: `ZAI_PLATFORM_KEY` secret → teaser chat runs glm-4.6 on the owner's coding endpoint (model+prices auto-published, M8.1 meters follow), +5 tests (216), v2.2.0 — owner: re-paste function, optionally set ZAI_PLATFORM_KEY |
| M8.3 Provider UX clarity + Paddle compliance pages | ✅ done | (this commit) | Settings→AI Model: providers relabeled ("GLM via z.ai — Coding Plan (recommended)" / "GLM via bigmodel.cn (mainland)"), per-model annotations in the picker (`modelNotes`: glm-4.5-flash "free tier", glm-4.6+ "needs balance" — explains owner's bigmodel.cn test results), relay providers show a static "managed automatically" endpoint instead of the raw `relay:` sentinel (no URL to paste), anti-password-manager attrs (`name`, `data-1p-ignore`, `data-lpignore`) on base-URL + key inputs, per-provider manual titles ("How to get a z.ai API key" etc.); new public pages for Paddle's website review — `/#/about`, `/#/contact` (gabor@deutschmeister.gaborscreation.space), `/#/terms`, `/#/privacy`, `/#/refund` (14-day policy) + footer links on every page (hash routes = always reachable, no 404 fallback needed); +2 tests, v2.3.0 |
| M9 Payments (hybrid) | ✅ code complete (v2.4.0) — owner deployment pending | (this commit) | **Paddle Billing sandbox build**: `paddle-checkout` Edge Function (plans catalog from `PADDLE_PRICE_MAP`, hosted-checkout transaction w/ `custom_data.user_id` + redirect only, customer-portal sessions), `paddle-webhook` Edge Function (Paddle-Signature HMAC verify ±300 s, idempotent via `billing_events`, subscription events → `ai_entitlements` grant/downgrade, credit-pack top-ups, **sandbox guard**: non-live events only touch `PADDLE_SANDBOX_TEST_USER`), migration 0003 (valid_until/source/tts_char_cap/cancel_at_period_end/paddle_customer_id + billing_events, RLS deny-all), ai-proxy v2 (lifetime teaser/credit + monthly allowance math mirroring `remainingBudgetWithMonthly`, per-plan HD-voice caps: free 20k taste / Basic 0 → 403 `hd-voice-not-in-plan` / Plus 150k, usage response publishes plan envelope), client: `src/llm/plans.ts` catalog (membership v3: Basic €3.99·€29.99, Plus €5.99·€49.99 ⭐, Pro dormant), `src/billing/paddle.ts` signature twin (unit-tested), Settings → **Account & Billing** (usage bars, monthly/annual pricing cards, checkout/manage redirects, BYO hint), +20 tests (218→238); **v2.4.1 fix**: Paddle.js overlay checkout (`src/billing/paddleClient.ts` — Paddle Billing has no API-hosted checkout page; new secret PADDLE_CLIENT_TOKEN + dashboard default payment link required, else 400 transaction_default_checkout_url_not_set), +6 tests (238→244); owner steps: run 0003, create sandbox products (4 price IDs), set default payment link, set 6 secrets, deploy 2 functions (webhook with JWT verify OFF), re-paste ai-proxy — then live E2E sandbox test |
| M9.5 Welcome & onboarding flow | ✅ v2.5.0 | `#/welcome`, auto-opens once per browser | Shipped: 4-step tour — (1) app intro (offline core, AI tutor, data ownership); (2) optional account reusing `authActions` (Google / email sign-in+sign-up / continue as guest; confirmation-email notice); (3) basics — name, A1–B2 level buttons, 5/10/15/20 daily-word goal → `patchProfile` (no schema change; C1/C2 wait for M11 content); (4) EXPLICIT AI choice per the owner vision: included-teaser card ($1 credit ≈700 turns, ~20k HD voice chars/mo, live meter, "what happens when it runs out", amber warning for guests that it needs a free account) vs own-key card (provider select + key → `setApiKey` + `patchSettings(provider/baseUrl/model)`, "Get one here" link, stored-only-in-browser note) + "Decide later in Settings" escape. Completion flag `dm-welcome-done` in localStorage (fresh browser on an existing account still sees the tour once); Dashboard redirects on first visit; "Replay welcome tour" button in Settings → Getting started (card now always renders, checklist ✔/○ kept). Satisfies "user needs to be informed" about automatic teaser usage |
| M9.6 BYO Supporter membership | ✅ v2.6.0 | (this commit) | Owner decisions 2026-09-21 (a: yearly · b: €11.99 · c: no HD-voice allowance) — supersedes the old "BYO free forever" lock: 30-day free trial from the first saved key (`settings.byoKeyFirstSeenAt`, optional Dexie column, no migration), then the yearly Paddle plan `byo-supporter` with ZERO platform allowances (chat AI + HD voice on the user's own keys — the system itself is the product). Pure gate in `entitlement.ts` (`byoAccess`/`byoTrialDaysLeft`, `resolveAiRoute` `byoAllowed` fall-through locked→platform/none) + `useByoGate()` hook + idempotent stamp in `patchApiKey`; UI: AiModelSection status banner (member ✓ / trial countdown / locked paywall), BillingSection Annual-tab Supporter card, welcome-tour & all "free forever" copy corrected; edge: `paddle-checkout` + `paddle-webhook` accept the plan (Supporter row replaces any prior plan). Owner: sandbox product €11.99/yr → priceId into `PADDLE_PRICE_MAP` (both functions) → re-paste BOTH. +8 tests (253→261) |
...
| 2026-09-24 | M8.2: tsc+vitest(216)+build+preview-smoke 200+secrets scan | ✅ green — z.ai returns as a first-class provider: ai-proxy BYO relay (/byo/zai-coding + /byo/zai-api → api.z.ai server-side; user key in x-dm-byo-key, forwarded once, never stored; host-locked routes, POST-only, 64 KB cap, 30 req/min/IP, 60 s timeout), default provider `glm-zai` (glm-4.6, relay sentinel baseUrl) + self-heal of pre-M2.2 api.z.ai rows onto it, adapter relay plumbing (resolveRelayBaseUrl, anon-JWT + x-dm-byo-key headers), optional `ZAI_PLATFORM_KEY` platform swap (teaser chat → glm-4.6 on the owner's coding endpoint; model+prices auto-published via M8.1), api.z.ai CORS re-verified still headerless, +5 tests (211→216), v2.2.0 — owner: re-paste ai-proxy in the dashboard + optionally set ZAI_PLATFORM_KEY |
| 2026-09-21 | M8.3: tsc+vitest(218)+build+preview-smoke 200+secrets scan + LIVE relay curl | ✅ green — provider UX de-confusion (details in M8.3 milestone row above), v2.3.0 |
| 2026-09-21 | M9: tsc+vitest(238)+build(1076.2 KB JS / 323.8 KB gzip)+preview-smoke 200+secrets scan+esbuild parse-check of all 3 Deno functions | ✅ green — payments code complete (v2.4.0): Paddle Billing sandbox build — `paddle-checkout` + `paddle-webhook` Edge Functions, migration 0003, ai-proxy monthly-allowance/per-plan-TTS upgrade, plan catalog + Account & Billing UI, signature twin unit-tested; live E2E pending owner sandbox deployment (products + 5 secrets + 2 function deploys + re-paste ai-proxy) |
| 2026-09-21 | M9 fix: tsc+vitest(244)+build+preview-smoke 200+secrets scan+esbuild parse-check of `paddle-checkout` | ✅ green — v2.4.1 overlay-checkout fix: owner's live test hit `400 transaction_default_checkout_url_not_set`; docs revealed Paddle Billing has NO API-hosted checkout page (checkout.url = `<default payment link>?_ptxn=…`, must be opened by Paddle.js), so billing now opens the server-created transaction as a Paddle.js overlay in-page (`src/billing/paddleClient.ts`, lazy script, Initialize-once, checkout.completed → close+meter poll; +6 tests); `paddle-checkout` returns {transactionId, clientToken, env, url}; new secret `PADDLE_CLIENT_TOKEN` + dashboard default payment link (M9_DEPLOY Step 2B/3, secrets now 6) — owner re-paste of `paddle-checkout` required |
| 2026-09-21 | M9 fix: tsc+vitest(245)+build+preview-smoke 200+secrets scan+esbuild parse-check of `paddle-checkout` | ✅ green — v2.4.5 root-cause fix for the "Something went wrong" popup INSIDE the overlay: the v2.4.4 `[PADDLE]` console diagnostics nailed it — `checkout.error api_error/validation — validation.no_validation_set` on `settings.customer.email` (/data/settings/customer/email): Paddle's transaction-checkout service rejects the customer-email prefill for checkouts opened by `transactionId` (prefill is only valid for items-based checkouts; the code isn't even in Paddle's public error reference). Removed `customer` + `allowLogout` from Checkout.open settings — the buyer types their email straight into Paddle's form; entitlement routing unaffected (paddle-webhook maps by custom_data.user_id set server-side). No Supabase change; also documented why sandbox Transactions can look empty (wrong dashboard/account) |
| 2026-09-21 | M9 sandbox E2E — owner-verified ✅ | ✅ green — with v2.4.5 the owner completed a real sandbox payment through the overlay (email+country typed into Paddle's form, test card accepted). Activation mapping confirmed by design and code: `paddle-checkout` stamps the Supabase `user_id` into the transaction's `custom_data` at creation, and `paddle-webhook` grants the entitlement by that id — checkout email/country/card never decide which account is activated (email = receipts/customer record, country = VAT as Paddle is merchant of record, card = payment only). Next: Paddle live account application, swap the 6 secrets to live values on go-live, optional Google TTS key, M10 |
| 2026-09-21 | M9.6: tsc+vitest(261)+build+secrets scan | ✅ green — BYO Supporter membership shipped (details in the M9.6 milestone row above); v2.6.0 |
| 2026-09-21 | M9.6 hardening: ai-proxy TTS voice whitelist | 🔒 deployed in repo — type=tts now accepts ONLY `de-DE-Neural2-[A-Z]` voice names (was any `de-DE-*`): a crafted request can no longer steer the platform key onto paid families (Studio $160/1M, Chirp 3 HD $30/1M — no free tier; Neural2 = 1M chars/mo free then $16/1M). In-app platform users already only see the curated Neural2 list, so behavior is unchanged for legit traffic. Owner must re-paste ai-proxy when setting PLATFORM_TTS_KEY |
| 2026-09-21 | M9.7: platform HD voice UX truth pass (v2.4.6) | ✅ green — owner testing exposed 4 issues: (1) the platform TTS resolver was installed unconditionally, so signed-out users saw the 5-voice included picker (speaking silently fell back — the UI lied); (2) keyless paid users' ▶Preview played the BROWSER voice because HdVoiceCard's `active` required a key (likely why included voices "sounded identical / male→female"); (3) the included list was a stale hardcode; (4) browser + HD lists both looked active. Fixes: configurePlatformTts(resolver, available?) gate (Layout: user ≠ null && ttsCharCap > 0), ai-proxy type=ttsVoices serves the live included German list (Neural2+Wavenet, whitelist-aligned, 1h cache), preview routes through hdTts with a clear error when inactive, Speech section greyes the inactive engine and badges say WHY HD is active (own key vs included with plan). The ~43-voice full list stays BYO-key-only (Studio $160/1M, Chirp 3 HD $30/1M excluded from platform billing). Owner action: re-paste ai-proxy + hard-refresh to 2.4.6 |
| 2026-09-21 | M9.5: welcome & onboarding flow (v2.5.0) | ✅ green — gate below; /welcome 4-step tour (intro → optional account via existing authActions: Google/email/guest → basics: name, A1–B2, daily goal → explicit AI choice: teaser properly introduced vs own key), once-per-browser auto-open (`dm-welcome-done` localStorage flag; Dashboard first-visit redirect), own-key path persists key + provider/baseUrl/model, "Replay welcome tour" in Settings → Getting started (card always renders now), unit tests for flag + validateBasics + normalizeName |
| 2026-09-21 | M9 fix: tsc+vitest(245)+build+preview-smoke 200+secrets scan+esbuild parse-check of `paddle-checkout` | ✅ green — v2.4.4 diagnostics: owner's test now gets PAST token/Initialize (overlay opens = token+env+transaction all good) but Paddle's iframe shows its own "Something went wrong" popup. PaddleEvent extended with {type,code,detail,errors[]} and the Initialize eventCallback now logs every `checkout.error`/`checkout.warning` to the console as `[PADDLE] …` lines (Paddle's documented first troubleshooting stop; payload per checkout-error/checkout-warning docs). M9_DEPLOY: new troubleshooting entry (Website approval of gabortardos.github.io in Checkout settings is the prime suspect on GitHub Pages + how to read the [PADDLE] console lines + draft-txn sanity check) |
| 2026-09-21 | M9 fix: tsc+vitest(245)+build+preview-smoke 200+secrets scan+esbuild parse-check of `paddle-checkout` | ✅ green — v2.4.3 SDK-API fix, found via the owner's live v2.4.2 error "[PADDLE] Unknown option parameter 'environment'" (the v2.4.2 diagnostics worked as designed): current Paddle.js CDN SDK removed `environment` from `Paddle.Initialize` — sandbox is now selected with `Paddle.Environment.set('sandbox')` called BEFORE Initialize (live = no call, production default; verified against current developer.paddle.com docs). Client-side only: no function re-paste, no secret change. initialize-failed guidance text de-misleading-ed (it is not always a token problem) |
| 2026-09-21 | M9 fix: tsc+vitest(245)+build+preview-smoke 200+secrets scan+esbuild parse-check of `paddle-checkout` | ✅ green — v2.4.2 diagnosability fix after owner's test "click Subscribe → silently lands on homepage": that was the redirect fallback firing (checkout.url IS our homepage?_ptxn — a dead end). Removed the silent redirect: overlay failures now return typed reasons surfaced in red under the plans (client-token-missing / script-load-failed / initialize-failed / open-failed); `paddle-checkout` fails fast with HTTP 500 when PADDLE_CLIENT_TOKEN is unset and stamps `fnVersion: 2` so the client can tell "old function deployed" from "secret missing"; landing with `?_ptxn=…` auto-resumes the stashed checkout overlay (main.tsx + sessionStorage stash); redirect only to genuine external pages (`isOwnSiteUrl`, +1 test) |
